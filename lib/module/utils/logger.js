...
export const makeLogger = logger => (level, logInfoProducer) => {
  ...
  if (enabledLogLevel) {
    let args = typeof logInfoProducer === 'function' ? logInfoProducer() : args;
    Array.isArray(args) || (args = [args]);
    logger(enabledLogLevel, args);
  }
};
export const consoleLogger = (level, args) => (level > WARNING ? console.error : console.log)(`[${levelText[level]}]`, ...args);
