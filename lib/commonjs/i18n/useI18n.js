...
const registerLocale = (locale, moduleName, loadedModule) => {
  ...
  for (let key in messages) {
    ...
    const id = "".concat(moduleName, ".").concat(key); //todo: be replaced with pre-compiled locale data
  ...
  const moduleKey = "".concat(locale, "/").concat(moduleName);
...
function useI18n(moduleName) {
  ...
  const state = (0, _useAsyncMemo.default)(async () => {
    if (moduleName) {
      ...
      if (cachedModule) {
        _Runtime.default.log('verbose', () => "Load locale data \"".concat(moduleKey, "\" from cache."));
